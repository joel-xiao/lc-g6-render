...
export default {
    ...
    click: (g6_graph, e, that, option) => {
        if (e.target.get('event-name') === 'node-collapsed') return; // that.isCachingState logic inlined/copied or referenced if we import helper?
        // original: if (that.isCachingState(g6_graph, e, that, option)) return;
        // We can use 'this' if bound correctly, but separating logic is safer.
        const event_name = e.target.get('event-name');
        if (event_name === 'node-collapsed') return;
