...
import { assignOptions, getLayout } from './graph-options'
import { getShape } from './register-shape-methods'
import { layoutEdges, layoutQuadraticEdges } from './register-layout-methods.js'
import { getRegisterEdge } from './register-edge'
import { getRegisterNode } from './register-node'
import { getRegisterCombo } from './register-combo'
import { registerLayout } from './register-layout'
import { getCustomBehavior } from './custom-behavior'
import { onEvent, getEvent } from './event'
import { joinDataMapKey } from './util'
import { mergeModel, calcNodeModelData } from './data-format-methods.js'
...
export class G6Graph {
  ...
  registerEdge() {
    const registerEdge = getRegisterEdge(this);
    Object.keys(registerEdge).forEach(key => {
      G6.registerEdge(key, registerEdge[key].edge, registerEdge[key].register);
    });
  }

  registerNode() {
    const registerNode = getRegisterNode(this);
    Object.keys(registerNode).forEach(key => {
      G6.registerNode(key, registerNode[key].node, registerNode[key].register);
    });
  }

  registerCombo() {
    const registerCombo = getRegisterCombo(this);
    
    Object.keys(registerCombo).forEach(key => {
      G6.registerCombo(key, registerCombo[key].combo, registerCombo[key].register);
    });
  }
